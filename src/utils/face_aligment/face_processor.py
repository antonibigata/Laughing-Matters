...
class FaceProcessor:
    ...
    def get_transform(self, image):
        ...
        warped_img, trans = self.warp_img(stable_points, self.mean_face[stablePntsIDs, :], image)
        return trans

    def process_image(self, image, landmarks=None):
        ...
        warped_img, trans = self.warp_img(
            stable_points, self.mean_face[stablePntsIDs, :], image, output_shape=self.img_size
        )
        return warped_img
    ...
    def normalise_face(self, video_input, landmarks_input=None, window_size=7):
        ...
        for frame_no in range(0, video.shape[0]):
            ...
            if frame_no + window_size <= video.shape[0]:
                ...
                out_video[frame_no], trans = self.warp_img(
                    avg_stable_points, self.mean_face[stablePntsIDs, :], video[frame_no], output_shape=self.img_size
                )
            else:
                if landmarks_input is None:
    ...
    def parse_landmarks_file(landmarks_file):
        video_landmarks = []
        if landmarks_file.endswith(".npy"):
            video_landmarks = np.load(landmarks_file)
            assert len(video_landmarks.shape) == 3, "Landmarks file should be 3D array"
        else:
