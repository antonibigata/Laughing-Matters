# https://github.com/jxbz/agd/tree/main
import math
import torch
...
class AGD:
    @torch.no_grad()
    def __init__(self, params, gain=1.0):  # you can try --gain 10.0 which we have found can accelerate training

        self.params = params
        self.depth = len(list(params))
        self.gain = gain

        for p in params:
            if p.dim() == 1:
                raise Exception("Biases are not supported.")
            if p.dim() == 2:
                orthogonal_(p)
            if p.dim() == 4:
