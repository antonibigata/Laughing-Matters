...
class DoG(Optimizer):
    ...
    def __init__(
        ...
        if init_eta is not None:
            ...
            if reps_rel <= 0.0:
                raise ValueError(
                    f"Invalid reps_rel value ({reps_rel}). Suggested value is 1e-6 "
        ...
        for group in self.param_groups:
            ...
            for tgroup in group.keys():
                ...
                if tgroup != "params":
                    ...
                    if (
                        isinstance(group[tgroup], list)
                        and len(group[tgroup]) > 0
                        and isinstance(group[tgroup][0], torch.Tensor)
                    ):
        ...
        if self._first_step:
            ...
            for p in group["params"]:
                if p.grad is not None:
                    G.append((p.grad.detach() ** 2).sum())
            # [(p.grad.detach() ** 2).sum() for p in group["params"]]
            group["G"] = torch.stack(G).sum() + group["eps"]
        else:
            ...
            for p in group["params"]:
                if p.grad is not None:
                    G.append((p.grad.detach() ** 2).sum())
            group["G"] += torch.stack(G).sum()
        assert group["G"] > 0, f'DoG cannot work when G is not strictly positive. got: {group["G"]}'
...
class LDoG(DoG):
    ...
    def _update_group_state(self, group, init):
        ...
        assert torch.all(group["G"] > 0).item(), f'DoG cannot work when g2 is not strictly positive. got: {group["G"]}'
